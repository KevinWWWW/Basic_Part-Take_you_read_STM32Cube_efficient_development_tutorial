...
void HAL_RTCEx_WakeUpTimerEventCallback(RTC_HandleTypeDef *hrtc)
{
	...
	if (HAL_RTC_GetTime(hrtc, &sTime, RTC_FORMAT_BIN) == HAL_OK) {
		...
		char str[40];
		/* Show Date */
		sprintf(str, "RTC Date= %4d-%2d-%2d", sDate.Year + 2000,
				sDate.Month, sDate.Date);
		lcd_show_str(0, 170, 16, str, RED);
		/* Show Time */
		sprintf(str, "RTC Time= %2d:%2d:%2d", sTime.Hours,
...
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *hrtc)
{
	triggerCntA ++;
	char str[40];
	sprintf(str, "Alarm A(xx:16:05) trigger: %d", triggerCntA);
...
void HAL_RTCEx_AlarmBEventCallback(RTC_HandleTypeDef *hrtc)
{
	triggerCntB ++;
	char str[40];
	sprintf(str, "Alarm B(xx:xx:30) trigger: %d", triggerCntB);
	lcd_show_str(0, 150, 16, str, RED);
}
